...
> 本文件是整个仓库的架构铁律与导航。子模块各有自己的 `CLAUDE.md`，进入某模块工作时同时遵守该模块规则。
> `ARCHITECTURE.md`（20KB）不必每次全读：**碰架构决策或踩到怪问题时**去看它的「雷区」清单（含实测过的编码/编译/端口坑）；末尾的里程碑表已过期，进度看 `docs/`。
...
**唯一真相源是 `docs/` 下这三份**（按状态分，改完代码要同步更新）：
- `docs/一、已完成（实机验证过、稳定可交运营）.md`
- `docs/二、待自测.md`
- `docs/三、待实现（没做  半成品  后置）.md`

> `docs/全局状态核对.md` 是 08.03 拆分前的旧快照，**已过期，别读别改**。
> `ARCHITECTURE.md` 末尾那张"实施里程碑"表同样过期（还把 Vue 前端/上新标着待办，实际早已跑通）——它有价值的部分是「雷区」清单，进度看 `docs/`。

大致现状：生图（云端）+ 上新/反风控（本地）+ Vue3 前端七页 + 产品替换/开品/补发三个专项模块均已实机跑通；剩下的是生图质量攻关与若干待自测项。
