...
**唯一真相源是 `docs/` 下这四份**（按状态分，改完代码要同步更新）：
- `docs/一、已完成（实机验证过、稳定可交运营）.md` — 表格只放一句话结论，字母节号
- `docs/二、待自测.md` — 只写【步骤】【期望】【日志】，批次名走天干地支序
- `docs/三、待实现（没做  半成品  后置）.md` — 欠账
- `docs/四、运维提醒与技术细节存档.md` — 运维提醒 + **长篇根因/量化证据**（按模块包进 `<details>`，字母块号）

> ⚠️ **长篇根因一律进第四份**，别写在「二、待自测」的批次说明里（08.06 犯过：当时第四份文件缺失，
> 分析被塞进待自测；已从旧快照重建）。旧快照 `docs/全局状态核对.md` 已于 08.06 **删除**（防误写）。
