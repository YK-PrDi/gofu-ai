...
public class WanImageAgent implements ImageGeneratorAgent {
    ...
    public boolean generateMulti(String prompt, List<String> refImagePaths,
        ...
        return generateWithSize(prompt, firstRef, whiteBgPath, outputPath, size, task);
    }
    ...
    private boolean generateWithSize(String prompt, String refImagePath, String whiteBgPath,
                                      String outputPath, String size, GenerationTask task) {
        // 【检查点1】任务开始前检查
        if (task != null && task.isCancelled()) {
            log.info("任务已取消，跳过万相API调用");
            return false;
        }
        ...
        try {
            ...
            String imageUrl = pollTask(client, taskId, task);
    ...
    public boolean generate(String prompt, String refImagePath, String whiteBgPath, String outputPath, GenerationTask task) {
        // 使用配置默认 size
        return generateWithSize(prompt, refImagePath, whiteBgPath, outputPath, config.getImageSize(), task);
    }
    ...
    public String generateUrl(String prompt) {
        ...
        try {
            ...
            log.info("万相任务已创建，task_id={}", taskId);
            return pollTask(client, taskId, null);
        } catch (IOException e) {
    ...
    private String pollTask(OkHttpClient client, String taskId, GenerationTask task) throws IOException, InterruptedException {
        Request request = new Request.Builder()
        ...
        for (int i = 0; i < 90; i++) {
            // 【检查点2】每次轮询前检查
            if (task != null && task.isCancelled()) {
                log.info("任务已取消，停止万相轮询");
                throw new InterruptedException("任务已取消");
            }

            // 【检查点3】等待期间分段检查（前5次2秒，之后3秒）
            int sleepSeconds = i < 5 ? 2 : 3;
            for (int j = 0; j < sleepSeconds; j++) {
                if (task != null && task.isCancelled()) {
                    log.info("任务已取消，中断万相轮询等待");
                    throw new InterruptedException("任务已取消");
                }
                Thread.sleep(1000);
            }
