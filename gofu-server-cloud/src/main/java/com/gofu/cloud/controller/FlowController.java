...
public class FlowController {
    ...
    private void genReplaceMains(ProductContext ctx, List<String> refImages, List<String> whiteBgs, int count, GenerationTask task) {
        ...
        // 08.07 三轮定稿：作废范围必须**只针对架体结构**，不能扩到整张参考图。
        //   一轮（原始）：只说"保留第二张的构图版式 / 把产品换成第一张的 / 自然融入原位置"，
        //     没有任何结构保真约束 → 出图是两款混血（雪人架白底图 + Jtutm 矩形波浪架竞品图，
        //     实测"波浪卡槽两端接不上外围圆框""右臂与波浪卡槽熔融成不规则多边形"——波浪卡槽
        //     来自竞品图那款、雪人架上本来没有，模型是在执行"融入原位置"）。
        //   二轮（改过头）：补了结构保真，但写成"第二张产品信息一律作废"+罗列一堆禁止照搬
        //     +"宁可与第二张的产品轮廓不重合" → 模型把参考图**整体**当不可信来源，直接弃用：
        //     实测抽卡出来全是同一视角同一构图同一背景，**锅盖砧板等被收纳物全消失**，
        //     等于完全没参考参考图。根因是我把两类内容混在一起否定了——
        //     "架子上摆了什么物品"属于**场景**，不属于"别款产品的结构"。
        //   三轮（本版）：按三类内容分别定去向，不再二元地信任/否定整张图：
        //     · 构图/背景/镜头/光影 → 照搬
        //     · 被收纳物（锅盖/砧板/勺子等）→ **照搬并必须保留**，只是改为架在本款的对应结构上
        //     · 架体自身结构（层数/杆件走向/卡槽样式/托盘形状/品牌标识）→ 只有这一类作废
        //   另删掉"宁可轮廓不重合"那句（它太强，直接鼓励模型脱离参考图），改为
        //   "结构不迁就轮廓，但位置/占比/朝向仍按第二张"——把脱离范围限定在结构本身。
        //   本方法拿不到 category（不接品类参数），故结构保真写成通用措辞，
        //   对齐架类 subjectLock 那套已验证有效的写法（1:1 复刻 + 逐个点数 + 左右对称），不用形容词。
        String prompt = "参考图共两张：第一张是**本款产品的白底图**，第二张是一张成品场景图（其中的产品是**另一款**，但场景与构图要照它画）。\n"
                + "【产品本体只看第一张】画面里的产品必须是第一张白底图里的那一个产品本身，"
                + "外形、结构、层数/格数、部件形状与数量、颜色、材质、比例一律 1:1 复刻第一张："
                + "成排重复的部件（挂钩/卡位/杆件/分格）对着第一张逐个点数、分毫不差；"
                + "成对部件左右对称（高度相同、角度镜像、长度一致）；不得改结构、不得增减部件、不得臆造第一张上没有的细节。\n"
                + "【第二张·除了架体结构以外全部照搬】请完整保留第二张的：构图布局、主体在画面中的位置与占画面比例、"
                + "镜头角度与俯仰、以及画面里的文字块位置——这些一概不要改。\n"
                + "【背景必须照第二张·严禁纯白】背景、场景、台面与墙面的材质与颜色、环境道具、光影氛围**全部照第二张原样画**，"
                + "这是一张**有真实使用场景的成品图**，不是白底图。**严禁**把背景画成纯白、纯色、渐变或影棚无缝底，"
                + "也不要另换成别的场景——第一张只提供产品本体，它的白色背景与本次背景无关、不要当成目标底色。\n"
                + "**尤其是架子上摆放的锅盖、砧板、勺子等被收纳物，以及场景里的其它道具，必须照第二张原样保留**"
                + "（数量、种类、颜色、摆放手法、在画面中的位置都跟着第二张），只是它们现在改为架在**本款产品**的对应卡位/层板/接水盘上，"
                + "接触与遮挡关系按本款产品的真实结构来画。**不要把它们删掉、也不要减少数量**——空架子不是本次要的结果。\n"
                + "【唯一不可照搬的：第二张那款架子自己的结构】它的层数、杆件走向、卡槽样式、托盘形状、品牌标识与本次无关，"
                + "本款该是什么结构就画什么结构、不迁就它的轮廓；但本款的位置、占画面比例、朝向仍按第二张摆。"
                + "结构上与第一张冲突时，无条件以第一张为准。";
    ...
    private String buildSeriesPrompt(String basePrompt, int currentIndex, int totalCount, String seriesPlan,
        ...
        // 08.06 删除原【结构强锁】段（322字）：与 subjectLock（品类库的【商品约束】）说的是同一件事
        //   ——"照白底图 1:1 复刻框架走线/杆件数/层数/卡位/接水盘/造型件"，两段措辞不同但信息重合；
        //   末句"前后遮挡/不穿模/不悬空"又与无条件追加的【三维层级与禁止穿模】重复第三遍。
        //   1500 字是官方给的精度上限，而主图原本 3159 字（208%）——同一个约束说四遍不会比说一遍更有效，
        //   只会挤掉真正需要被读到的指令。故此处不再重复，structLock 现仅用于 buildAngleConstraint 的分支判定。
        ...
        if (fromLib) {
            // 08.06 压缩免责（-320字）：原来三句话反复申明"这是别款/禁止照搬/以白底图为准"。
            //   而 08.06 已从源头把污染字段删掉（assembleComposition 只取版式字段，
            //   核心主体/视觉记忆点/风格与画质 不再进 prompt），围栏要防的东西少了一大半，
            //   剩下的用一句说清即可。
            // 08.06 第二轮：「参考图/参考图1」那句(44字)改为**按需追加**。
            //   分级品类(锅盖架/置物架/支架挂钩等 63 行)经第二轮裁字段后已 0 行含"参考图"字样，
            //   对它们说"它提到的参考图不是你收到的图"是在凭空引入一个 prompt 里不存在的概念——
            //   反而可能让模型去找第二张参考图。只有旧 baked 格式(花洒喷头 41 行全部含"参考图N")
            //   才真需要这句消歧，故改成检测 base 实际内容后再决定加不加。
            String refDisclaim = base.contains("参考图")
                    ? "它提到的「参考图/参考图N」不是你收到的任何一张图（你收到的第一张是本款白底图，只定产品、不定版式）。"
                    : "";
            sb.append(LIB_FENCE_MARK).append('\n')
              .append("下面这段文字是从**另一款商品**的图上反推的，只用来定**版式**："
                    + "视角俯仰、主体在画面中的位置与占比、信息栏分区、功能小图数量与排布、标题与标签位置、"
                    + "被收纳物的摆放手法。其中出现的产品名称、颜色、材质、层数、部件形状、具体摆了什么物品，"
                    + "一律与本次无关、禁止照搬；与白底图冲突时无条件以白底图为准。")
              .append(refDisclaim).append('\n')
        ...
        // 08.06：totalCount==1 时整段跳过——只出一张图，"与第1张保持一致""各张可明显不同"
        //   这类跨张约束全是废话（库命中时 281 字、现编时 380 字+角度约束）。
        //   1500 字是精度上限，单张生成时白扔两三百字在无意义的段落上，会挤掉真正要被读到的指令。
        if (totalCount <= 1) {
            // 单张：只补一句产品原生文字要求（这条与张数无关，是唯一有用的部分）
            sb.append("\n【产品原生文字】产品本体上的 LOGO/品牌名/型号/标签/刻度须清晰正确，不得模糊、变形或消失。\n");
        } else if (fromLib) {
