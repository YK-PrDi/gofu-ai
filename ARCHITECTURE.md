...
### ADR-005 前端框架待定（2026-06-20，**待用户确认**）
...
**待决**：预览页交互极重（改价联动/字段锁定/局部重绘），是否用 Vue3 仅重写预览页、旧页面原生 JS 共存。**实现到 M6 前必须定**。
...
| M6 预览页 | 统一前端工作台+合流预览+微调回写+字段锁定（前端框架ADR-005此前必须定） | 点击≤3次，改价联动/局部重绘/锁定生效 | 待办 |
